...
class Score(object):
    ...
    #méthode de stockage :
    def toFile(self, nomFichier):
        f = open(nomFichier,"w")
        #création d'une liste
        d = {"NomJoueur": self.nomJoueur, "Score": self.nbPoints}
        json.dump(d,f)
        f.close()
    
    #methode de lecture du fichier :
    def fromFile(self, nomFichier):
        f = open(nomFichier, "r")
        d = json.load(f)
        print(d)
        f.close()
...
class Game(object):
    ...
    def install_in(self, canvas):
        new_player = tkMessageBox.askokcancel(title="player", message="Etes-vous un nouveau joueur ?")
        if(new_player == True):
            tk.Label(canvas, text="Nom du Joueur").pack()
            nom = Entry(canvas)
            nom.pack()
            score = None
            new = Score(str(nom), score)
            tk.Button(canvas, text="ok", command=new.toFile("Scores.json")).pack()
            self.score.append(new)
            tk.Button(canvas, text="jouer !", command=self.install_all(canvas)).pack()
        else:
            self.install_all(canvas)
            for scores in self.score :
                tk.Button(canvas, text=score.fromFile("Scores.json"), command=self.install_all(canvas))

    def install_all(self, canvas):  #on demande le canvas pour les autres classes
